...
function NewMessage({
  ...
  const [messageImportance, setMessageImportance] =
  ...
  const recipientsToValidRecipientFormat = (
    emails: string
  ): NullableOption<Recipient[]> => {
    if (emails) {
  ...
  const handleSend = (send: boolean) => () => {
    ...
    if (ccRecipients) {
      extraProps["ccRecipients"] =
        recipientsToValidRecipientFormat(ccRecipients);
    }
    if (bccRecipients) {
      extraProps["bccRecipients"] =
        recipientsToValidRecipientFormat(bccRecipients);
    }
          ...
          <IconButton
            ...
            onClick={() =>
              messageImportance == "normal"
                ? setMessageImportance("high")
                : setMessageImportance("normal")
            }
          >
