...
export async function getContacts(): Promise<Contact[]> {
  const response: PageCollection = await graphClient!.api('/me/contacts')
    .get();
...
export async function getContactFolders(): Promise<Contact[]> {
  const response: PageCollection = await graphClient!.api('/me/contactFolders')
    .get();
...
export async function postContact(contact: Contact): Promise<Contact> {
  return await graphClient!.api('/me/contacts')
    .post(contact);
}

export async function deleteContact(contactId: string): Promise<void> {
  return await graphClient!.api('/me/contacts/' + contactId)
    .delete();
}
...
export async function patchContact(contact: Contact): Promise<Contact> {
  ...
  return await graphClient!.api('/me/contacts/' + contact.id)
    .patch(contact);
}